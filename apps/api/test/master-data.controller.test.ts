...
test("master data controllers delegate CRUD calls to services", async () => {
  const productsCalls = { id: "", payload: {} as Record<string, unknown>, removedId: "" };
  const productsController = new ProductsController({
    findAll: async () => [{ id: "product-1", sku: "SKU-001" }],
  ...
  assert.equal((await productsController.findAll())[0]?.sku, "SKU-001");
